...
type IScApi interface {
	GetAuctionHistory(itemId, region string, limit int) (models.AuctionHistoryResponse, error)
}
...
func (s *ScApi) GetAuctionHistory(itemId, region string, limit int) (models.AuctionHistoryResponse, error) {
	strUrl := fmt.Sprintf("%s/%s/auction/%s/history", s.domain, region, itemId)
	req, err := http.NewRequest(http.MethodGet, strUrl, nil)
	if err != nil {
		return models.AuctionHistoryResponse{}, err
	}
