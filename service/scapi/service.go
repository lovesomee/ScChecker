...
func NewScApi(logger *zap.Logger, config config.Settings, httpClient *http.Client) *ScApi {
	return &ScApi{logger: logger, config: config, httpClient: httpClient}
}
...
func (s *ScApi) GetAuctionHistory(ctx context.Context, itemId, region string, limit int) (models.AuctionHistoryResponse, error) {
	strUrl := fmt.Sprintf("https://%s/%s/auction/%s/history", s.config.Stalcraft.DomainApi, region, itemId)
