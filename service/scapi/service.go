...
type IScApi interface {
	GetAuctionHistory(itemId, region string, limit int) (AuctionHistoryResponse, error)
}
...
func NewScApi(httpClient *http.Client, domain string) *ScApi {
	return &ScApi{httpClient, domain}
}
...
func (s *ScApi) GetAuctionHistory(itemId, region string, limit int) (AuctionHistoryResponse, error) {
	strUrl := fmt.Sprintf("%s/%s/auction/%s/history", s.domain, region, itemId)
	req, err := http.NewRequest(http.MethodGet, strUrl, nil)
	if err != nil {
		return AuctionHistoryResponse{}, err
	}
	...
	var auctionHistoryResponse AuctionHistoryResponse
